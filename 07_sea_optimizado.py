#!/usr/bin/env python3
# ============================================================
# ARQUITECTURA SEA OPTIMIZADA
# Con cache de claves y procesamiento por lotes
# ============================================================
...
class CryptoEngineOptimized:
    ...
    def _get_session_key(self, context):
        key_id = f"{context.device.value}_{context.sensitivity.value}"
        if key_id not in self._session_key_cache:
            self._session_key_cache[key_id] = os.urandom(32)
        return self._session_key_cache[key_id]
    ...
    def encrypt_batch(self, data_batch, context):
        """Procesamiento por lotes - OPTIMIZACION CLAVE"""
        results = []
        algo = self.select_algorithm(context)
        ...
        self.stats[f"{algo}_batch"] = self.stats.get(f"{algo}_batch", 0) + len(data_batch)
        return results
...
def run_demo():
    print("=" * 60)
    print("  ARQUITECTURA SEA OPTIMIZADA")
    print("  Con Cache de Claves y Procesamiento por Lotes")
    print("=" * 60)

    engine = CryptoEngineOptimized()
    ...
    print("\n[1] SELECCION DINAMICA CON CACHE DE CLAVES")
    print("-" * 60)
    ...
    print("\n[2] PROCESAMIENTO POR LOTES (OPTIMIZACION)")
    print("-" * 60)

    batch_data = [b"Mensaje lote " + str(i).encode() for i in range(10)]
    context = scenarios[0]
    start = time.perf_counter()
    results = engine.encrypt_batch(batch_data, context)
    elapsed = time.perf_counter() - start

    print(f"Procesados {len(batch_data)} mensajes en lote")
    print(f"Tiempo total: {elapsed*1000:.2f} ms")
    print(f"Promedio por mensaje: {elapsed*1000/len(batch_data):.2f} ms")
    ...
    print(f"\n[4] AHORRO ENERGETICO ESTIMADO")
    print(f"  Consumo tradicional: 100%")
    print(f"  Consumo con SEA optimizado: {energia_sea*100:.0f}%")
    print(f"  Ahorro: {(1-energia_sea)*100:.0f}%")

    print("\n[5] OPTIMIZACIONES IMPLEMENTADAS")
    print("-" * 60)
    print("  Cache de claves de sesion")
    print("  Procesamiento por lotes")
    print("  Uso de librerias certificadas")
    print("  Seleccion dinamica por contexto")
    print("=" * 60)
